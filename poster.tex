...
\begin{itemize}
    \item We modelled the problem as a classification problem where the learning objective is the discrete CPC scores (1 - 5). The final clinical outcome prediction is obtained via the mapping: CPC = 1 or 2 $\to$ ``Good outcome''; CPC = 3, 4, or 5 $\to$ ``Poor outcome''.
    \vspace{-0.2cm}
...
\headerbox{Preprocess Pipeline}{name=data, column=1, row=1, span=2, below=mtl}{
% finished
\begin{itemize}
    \item Butterworth bandpass filtering of order 4 and cutoff frequencies 0.5 - 30 Hz.
    \vspace{-0.2cm}
    \item Resampling to 100 Hz.
    \vspace{-0.2cm}
